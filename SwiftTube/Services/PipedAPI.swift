...
final class PipedAPI: ObservableObject {
    ...
    private func parseVideoStreams(from json: [String: Any]) throws -> VideoStreams {
        ...
        // Parse HLS URL if available
        if let hlsString = json["hls"] as? String,
           let url = URL(string: hlsString) {
            hlsURL = url
            print("Found HLS stream: \(hlsString)")
        }
        
        // Parse video streams
        if let videoArray = json["videoStreams"] as? [[String: Any]] {
            print("Found \(videoArray.count) video streams")
            
            for videoData in videoArray {
                do {
                    if let urlString = videoData["url"] as? String,
                       let url = URL(string: urlString),
                       let quality = videoData["quality"] as? String,
                       let format = videoData["format"] as? String {
                        
                        let bitrate = videoData["bitrate"] as? Int
                        let isVideoOnly = videoData["videoOnly"] as? Bool ?? true
                        let codec = videoData["codec"] as? String
                        ...
                        print("Added video stream: \(quality) \(format)")
                    }
                } catch {
                    print("Failed to parse video stream: \(error)")
                }
            }
        }
        
        // Parse audio streams
        if let audioArray = json["audioStreams"] as? [[String: Any]] {
            print("Found \(audioArray.count) audio streams")
            
            for audioData in audioArray {
                do {
                    if let urlString = audioData["url"] as? String,
                       let url = URL(string: urlString),
                       let format = audioData["format"] as? String {
                        
                        let bitrate = audioData["bitrate"] as? Int
                        let quality = audioData["quality"] as? String ?? "Audio"
                        let codec = audioData["codec"] as? String
                        ...
                        print("Added audio stream: \(format) (\(bitrate ?? 0)kbps)")
                    }
                } catch {
                    print("Failed to parse audio stream: \(error)")
                }
            }
        }
        
        print("Parsed streams: \(videoStreams.count) video, \(audioStreams.count) audio")
